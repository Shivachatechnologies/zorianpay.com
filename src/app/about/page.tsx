        ...
        <Container>
          ...
          <div className="mt-16 space-y-6">
            {timeline.map((item) => (
              <div
                key={item.year}
                className="flex flex-col gap-4 rounded-2xl border border-border bg-surface p-6 sm:flex-row sm:items-start sm:gap-8"
              >
          ...
          <div className="mt-16 grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {team.map((member) => (
              <Card key={member.name} className="h-full">
                <div
                  className={`flex h-14 w-14 items-center justify-center rounded-full bg-gradient-to-br ${member.color} text-base font-bold text-black`}
                >
                  {member.initials}
                </div>
                <h3 className="mt-4 text-lg font-semibold text-foreground">
